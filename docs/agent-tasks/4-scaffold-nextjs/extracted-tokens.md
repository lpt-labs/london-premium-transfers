...
## Tailwind v4 `@theme` integration decisions

These choices govern how `lib/design-tokens.ts` (commit `a982b90`) gets reflected into Tailwind's CSS-first config so utilities like `bg-paper`, `text-ink-soft`, `text-accent`, `rounded-l`, `p-pad` work in TSX.

### How `@theme` becomes utilities

Tailwind v4 generates utility classes by reading CSS custom properties declared inside an `@theme {}` block. The namespace prefix on the variable determines which utilities are produced:

| `@theme` declaration | Generated utilities |
|---|---|
| `--color-paper: #fafaf7` | `bg-paper`, `text-paper`, `border-paper`, `fill-paper`, `stroke-paper`, … |
| `--font-sans: 'Geist', …` | `font-sans` (sets `font-family`) |
| `--radius-l: 18px` | `rounded-l` and corner variants (`rounded-tl-l`, `rounded-br-l`, …) |
| `--shadow-s2: …` | `shadow-s2` |
| `--spacing-pad: clamp(20px, 4vw, 56px)` | `p-pad`, `m-pad`, `gap-pad`, `w-pad`, `h-pad`, … |

There's no manual binding step — the variable name *is* the utility name (with the namespace prefix stripped).

### Sync model — convention, not generation

Choice: **A. Hand-write `@theme`** in `app/globals.css` so the literal values from `lib/design-tokens.ts` appear directly in CSS. Rejected:

- *B. Build-time generator script*: ~100 lines of TS to keep ~30 lines of CSS in sync. Cost > benefit at the current scale; revisit if the token surface grows.
- *C. CSS as source of truth, derive TS*: loses TS literal types for token names and creates an inverse drift problem.

Risk: the two files can drift. Mitigations:

- Header comments in both files explicitly say "if you change this, also update [the other file]" and link by path.
- Plan TODO: add a CI lint step in a later PR (sibling to `plan-gate` / `drift-check`) that parses both files and fails the build if a token name or value differs.
- Until that check exists, the manual review of any token-touching PR is the safeguard. Reviewer responsibility.

### Token naming preserved from source

Decision: keep the **source's own token names** verbatim in both TS keys (camelCase) and CSS variables (kebab-case). No "brand-" prefix, no rename to align with Tailwind defaults. Result:

| Group | CSS variable | Utility |
|---|---|---|
| Colours | `--color-paper`, `--color-ink-soft`, `--color-accent-2`, … | `bg-paper`, `text-ink-soft`, `bg-accent-2` |
| Fonts | `--font-sans`, `--font-serif`, `--font-mono` | `font-sans`, `font-serif`, `font-mono` |
| Radii | `--radius-s`, `--radius-m`, `--radius-l`, `--radius-xl`, `--radius-full` | `rounded-s`, `rounded-m`, `rounded-l`, `rounded-xl`, `rounded-full` |
| Shadows | `--shadow-s1`, `--shadow-s2`, `--shadow-s3` | `shadow-s1`, `shadow-s2`, `shadow-s3` |
| Spacing | `--spacing-pad`, `--spacing-section`, `--spacing-nav-height` | `p-pad`, `p-section`, `h-nav-height` |
| Layout | `--container-maxw` *(custom; referenced via `var()` not via a `container-*` utility)* | n/a |

Trade-off accepted: this overrides a few Tailwind defaults (e.g. `--radius-xl` swaps from `0.75rem` to `28px`). That's intentional — the brand vocabulary wins over Tailwind's vocabulary inside this codebase.

### Font loading via `next/font/google`

The scaffolder already wired `Geist` and `Geist_Mono` through `next/font/google` in `app/layout.tsx` and exposes them as `--font-geist-sans` / `--font-geist-mono`. We adapt:

- Keep `Geist` → expose as `--font-sans` (not `--font-geist-sans`) so `@theme` can reference it directly.
- **Drop** `Geist_Mono` → replace with `JetBrains_Mono` → expose as `--font-mono`.
- **Add** `Instrument_Serif` (italic enabled) → expose as `--font-serif`.
- Drop the Google Fonts `<link>` block from the design source — `next/font` self-hosts the files, adds `font-display: swap`, avoids a render-blocking external request, and produces no CLS.

Within `@theme`, font tokens reference the variables `next/font` injects:
...
This indirection is the canonical pattern in the Tailwind+Next docs and is what the scaffold's existing `@theme inline` block was doing.

### Cross-links

- `lib/design-tokens.ts` — typed source of truth (commit `a982b90`).
- `app/globals.css` — Tailwind `@theme` mirror (next commit).
- `app/layout.tsx` — `next/font/google` setup (next commit).
