...
- **Steps:**
  ...
  4. Add `.github/agents/implementer.agent.md` — DORMANT Copilot custom agent. Frontmatter per the gh custom-agent spec at implementation time (`applyTo: [app/**, components/**, lib/**]`, `tools: [read_file, write_file, search_files, run_tests]`). Body mirrors the Claude subagent's prose but uses Copilot's tool vocabulary. Header comment marks DORMANT + points to `docs/COPILOT_STUDY/org-custom-agents.md`.
  5. Add `.github/agents/a11y-reviewer.agent.md` — DORMANT, read-only Copilot agent. Frontmatter `applyTo: [**/*.tsx, **/*.html]`, `tools: [read_file, search_files]`. Same content shape as the Claude file. Header comment marks DORMANT.
