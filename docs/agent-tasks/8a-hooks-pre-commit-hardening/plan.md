...
- **Steps:**
  ...
  4. Add `husky` and `lint-staged` as devDependencies in `package.json` (pinned to current major versions per the action-pinning convention extended to deps — `husky@^9`, `lint-staged@^15` or whatever the current stable majors are at time of writing). Add a `prepare` script that runs `husky` on `pnpm install`. Add a `lint-staged` config block inline that maps `*.{ts,tsx,js,jsx,mjs,cjs}` to `eslint --fix --max-warnings 0`. The `--max-warnings 0` flag is essential: without it, ESLint exits 0 when it emits only warnings (no errors), and the pre-commit hook lets the commit through. Next.js's default `eslint-config-next` sets common rules like `@typescript-eslint/no-unused-vars` to `warn`, so a "warnings are fine" hook would silently accept unused imports, dead code, and similar low-severity issues. With `--max-warnings 0`, any warning fails the run, blocking the commit. Developers who genuinely need to commit through a warning can use `git commit --no-verify` (documented in the README). Run `pnpm install` locally to refresh `pnpm-lock.yaml`.
